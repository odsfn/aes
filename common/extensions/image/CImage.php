...
class CImage {
	...
	public function __construct($image, $config = NULL)
	{
		...
		if ( ! isset(CImage::$allowed_types[$image_info[2]]))
		...
		(
			...
			'height' => $image_info[1],
			'type' => $image_info[2],
			'ext' => CImage::$allowed_types[$image_info[2]],
	 ...
	 * wish to use height as master dim, set $image->master_dim = CImage::HEIGHT
	 * This method is chainable.
	 *
	 * @throws  Kohana_Exception
	 * @param   integer  width
	 * @param   integer  height
	 * @param   integer  one of: CImage::NONE, CImage::AUTO, CImage::WIDTH, CImage::HEIGHT
	 * @return  object
	 */
	...
	protected function valid_size($type, & $value)
	{
		...
		switch ($type)
		{
			...
			break;
			case 'master':
				if ($value !== CImage::NONE AND
				    $value !== CImage::AUTO AND
				    $value !== CImage::WIDTH AND
				    $value !== CImage::HEIGHT)
					return FALSE;
			break;
		}
