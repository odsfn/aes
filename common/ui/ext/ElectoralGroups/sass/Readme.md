...
folder are mapped to the application's root namespace, 'ElectoralGroups'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "ElectoralGroups"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'ElectoralGroups' in the same way
as `"ElectoralGroups/sass/src"`.
