...
folder are mapped to the application's root namespace, 'GlobalVoterGroups'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "GlobalVoterGroups"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'GlobalVoterGroups' in the same way
as `"GlobalVoterGroups/sass/src"`.
