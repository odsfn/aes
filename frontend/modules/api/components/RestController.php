...
class RestController extends ERestController
{
    ...
    private function setModelAttributes($model, $data)
    {
        foreach ($data as $var => $value) {
            if (($model->hasAttribute($var) || isset($model->metadata->relations[$var])) && !in_array($var, $this->restrictedProperties)) {
                ...
                if (!isset($_GET['extjs'])   //We should not set extjs client id
                        || $model->metaData->tableSchema->primaryKey != $var || !strstr($value, '-')
                ) {
                    $model->$var = $value;
                }
            } else {
    ...
    public function doRestList()
    {
        ...
        $this->outputHelper(
                'Records Retrieved Successfully', $results, $resultsCount
        );
    }
